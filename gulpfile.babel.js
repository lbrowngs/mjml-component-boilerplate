...
const watchedTemplates = walkSync('./templates')
...
const compile = () => {
  return gulp
    ...
    .on('end', () => {
      ...
      watchedTemplates.forEach((templatePath) => {
        fs.readFile(path.normalize(templatePath), 'utf8', (err, data) => {
          if (err) throw err
          const result = mjml2html(data)
          fs.writeFileSync(path.normalize(templatePath.split('.')[0] + '.html'), result.html)
        })
      })
    })
}
